...
void Thread::Fork(VoidFunctionPtr func, void *arg)
{
    ...
    kernel->currentProcess->sh->ReadyToRun(this);
    if (kernel->currentProcess->prior != -1)
        cout << "Thread is forked" << endl;
...
void Thread::Begin()
{
    ...
    kernel->currentProcess->sh->CheckToBeDestroyed();
...
void Thread::Yield()
{
    ...
    nextThread = kernel->currentProcess->sh->FindNextToRun();
    if (nextThread != NULL)
    {
        kernel->currentProcess->sh->ReadyToRun(this);
        kernel->currentProcess->sh->Run(nextThread, FALSE);
    }
...
void Thread::Sleep(bool finishing)
{
    ...
    status = BLOCKED;
    while ((nextThread = kernel->currentProcess->sh->FindNextToRun()) == NULL)
        kernel->interrupt->Idle(); // no one to run, wait for an interrupt

    // returns when it's time for us to run
    kernel->currentProcess->sh->Run(nextThread, finishing);
}
